...
class ResNetBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = conv_type(in_planes, width, kernel_size=1, bias=False)
        self.bn1 = norm_type(width)
        self.conv2 = conv_type(width, width, kernel_size=3, stride=stride, padding=1, groups=cardinality, bias=False)
        self.bn2 = norm_type(width)
        self.conv3 = conv_type(width, outplanes, kernel_size=1, bias=False)
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv_type: type[nn.Conv1d | nn.Conv2d | nn.Conv3d] = Conv[Conv.CONV, spatial_dims]
        self.norm_type: type[nn.BatchNorm1d | nn.BatchNorm2d | nn.BatchNorm3d] = Norm[Norm.BATCH, spatial_dims]
        self.pool_type: type[nn.MaxPool1d | nn.MaxPool2d | nn.MaxPool3d] = Pool[Pool.MAX, spatial_dims]
        self.avgp_type: type[nn.AdaptiveAvgPool1d | nn.AdaptiveAvgPool2d | nn.AdaptiveAvgPool3d] = Pool[
        ...
        self.bn1 = self.norm_type(self.in_planes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = self.pool_type(kernel_size=3, stride=2, padding=1)
        ...
        self.layer4 = self._make_layer(block, block_inplanes[3], layers[3], spatial_dims, shortcut_type, stride=2,
                                       base_width=base_width, cardinality=cardinality)
        self.avgpool = self.avgp_type(block_avgpool[spatial_dims])
    ...
    def initial_weights(self):
        for m in self.modules():
            ...
            elif isinstance(m, self.norm_type):
    ...
    def _make_layer(
        ...
        layers = [
            block(in_planes=self.in_planes, planes=planes, spatial_dims=spatial_dims, stride=stride,
                  downsample=downsample, **kwargs)
        ]

        self.in_planes = planes * block.expansion
        for _i in range(1, blocks):
            layers.append(block(self.in_planes, planes, spatial_dims=spatial_dims, **kwargs))
    ...
    def logits(self, x: torch.Tensor, last_feature: bool):
        x = self.avgpool(x)
    ...
    def forward(self, x: torch.Tensor, last_feature: bool = False) -> torch.Tensor:
        x = self.forward_features(x)
        x = self.logits(x, last_feature)
        return x
...
def wideresnet50(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    return _resnet("wideresnet50", ResNetBottleneck, [3, 4, 6, 3], get_inplanes(), pretrained, progress, base_width=128,
                   **kwargs)


def resnext50(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    return _resnet("resnext50", ResNetBottleneck, [3, 4, 6, 3], get_inplanes(), pretrained, progress, cardinality=32,
                   base_width=4, **kwargs)
...
if __name__ == '__main__':
    models = [resnet50(n_input_channels=1, spatial_dims=3, num_classes=2),
              resnext50(n_input_channels=1, spatial_dims=3, num_classes=2),
              wideresnet50(n_input_channels=1, spatial_dims=3, num_classes=2),
              ]

    for model in models:
        # print(sum(p.numel() for p in model.parameters() if p.requires_grad))
        out = model(torch.rand(3, 1, 224, 224, 112), last_feature=True)
        print(out[1].shape)
