...
class EfficientNet(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, last_feature: bool = False):
        """
        Args:
            inputs: input should have spatially N dimensions
            ``(Batch, in_channels, dim_0[, dim_1, ..., dim_N])``, N is defined by `dimensions`.
            last_feature: return last feature
        Returns:
...
class BlockArgs(NamedTuple):
    ...
    def from_string(block_string: str):
        ...
        stride_check = (
                ...
                or (len(options["s"]) == 3 and options["s"][0] == options["s"][1] and options["s"][0] == options["s"][
            2])
        )
        if not stride_check:
...
if __name__ == '__main__':
    model = EfficientNetBN("efficientnet-b0", in_channels=1, spatial_dims=3, num_classes=2)
    import torch
